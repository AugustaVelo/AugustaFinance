...
contract LendPoolLiquidator is Initializable, ILendPoolLiquidator, LendPoolStorage, ContextUpgradeable {
     ...
     *
     */
    function auction(address nftAsset, uint256 nftTokenId, uint256 bidPrice, address onBehalfOf) external override {
        ...
        if (loanData.state == DataTypes.LoanState.Active) {
            ...
            vars.auctionEndTimestamp =
                loanData.bidStartTimestamp + (nftData.configuration.getAuctionDuration() * 1 days);
        ...
        ILendPoolLoan(vars.loanAddress).auctionLoan(
            vars.initiator, vars.loanId, onBehalfOf, bidPrice, vars.borrowAmount, reserveData.variableBorrowIndex
        );
        ...
        reserveData.updateInterestRates(loanData.reserveAsset, reserveData.aTokenAddress, 0, 0);
     ...
     *
     */
    function redeem(address nftAsset, uint256 nftTokenId, uint256 amount) external override returns (uint256) {
        ...
        vars.maxRepayAmount =
            vars.borrowAmount.percentMul(PercentageMath.PERCENTAGE_FACTOR - PercentageMath.TEN_PERCENT);
        if (vars.repayAmount > vars.maxRepayAmount) {
            vars.repayAmount = vars.maxRepayAmount;
        }

        ILendPoolLoan(vars.poolLoan).redeemLoan(
            vars.initiator, vars.loanId, vars.repayAmount, reserveData.variableBorrowIndex
        );
        ...
        if (loanData.bidderAddress != address(0)) {
            ...
            IERC20Upgradeable(loanData.reserveAsset).safeTransferFrom(
                vars.initiator, loanData.bidderAddress, vars.bidFine
            );
        }
     ...
     *
     */
    function liquidate(address nftAsset, uint256 nftTokenId, uint256 amount) external override returns (uint256) {
        ...
        ILendPoolLoan(vars.poolLoan).liquidateLoan(
            loanData.bidderAddress, vars.loanId, nftData.bNftAddress, vars.borrowAmount, reserveData.variableBorrowIndex
        );

        IDebtToken(reserveData.debtTokenAddress).burn(
            loanData.borrower, vars.borrowAmount, reserveData.variableBorrowIndex
        );

        // update interest rate according latest borrow amount (utilizaton)
        reserveData.updateInterestRates(loanData.reserveAsset, reserveData.aTokenAddress, vars.borrowAmount, 0);

        // transfer extra borrow amount from liquidator to lend pool
        if (vars.extraDebtAmount > 0) {
            IERC20Upgradeable(loanData.reserveAsset).safeTransferFrom(
                vars.initiator, address(this), vars.extraDebtAmount
            );
        }

        // transfer borrow amount from lend pool to aToken, repay debt
        IERC20Upgradeable(loanData.reserveAsset).safeTransfer(reserveData.aTokenAddress, vars.borrowAmount);
