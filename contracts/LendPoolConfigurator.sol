...
contract LendPoolConfigurator is Initializable, ILendPoolConfigurator {
    ...
    function _initReserve(ILendPool pool_, InitReserveInput calldata input) internal {
        ...
        emit ReserveInitialized(
            input.underlyingAsset, bTokenProxyAddress, debtTokenProxyAddress, input.interestRateAddress
        );
    }
    ...
    function _initNft(ILendPool pool_, IBNFTRegistry registry_, InitNftInput calldata input) internal {
    ...
    function updateAToken(UpdateATokenInput calldata input) external onlyPoolAdmin {
        ...
        _upgradeTokenImplementation(reserveData.aTokenAddress, input.implementation, input.encodedCallData);
     ...
     *
     */
    function configureNftAsAuction(address asset, uint256 redeemDuration, uint256 auctionDuration, uint256 redeemFine)
        external
        onlyPoolAdmin
    {
        ILendPool cachedPool = _getLendPool();
    ...
    function _upgradeTokenImplementation(address proxyAddress, address implementation, bytes memory encodedCallData)
        internal
    {
        BendUpgradeableProxy proxy = BendUpgradeableProxy(payable(proxyAddress));
    ...
    function _checkReserveNoLiquidity(address asset) internal view {
        ...
        uint256 availableLiquidity = IERC20Upgradeable(asset).balanceOf(reserveData.aTokenAddress);
